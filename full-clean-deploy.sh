...
# 4️⃣ Remove old backups or port-forward logs
echo "🗑️ Removing old backups and port-forward logs..."
rm -rf ~/kube-credential-system/.old-backups
rm -rf ~/kube-credential-system/kube-credential-k8s/port-forward-logs
mkdir -p ~/kube-credential-system/kube-credential-k8s/port-forward-logs
...
DEPLOY_SCRIPT="~/kube-credential-system/kube-credential-k8s/deploy.sh"
...
WATCHDOG_SETUP="~/kube-credential-system/setup-watchdog-systemd.sh"
if [ -f "$WATCHDOG_SETUP" ]; then
    chmod +x "$WATCHDOG_SETUP"
    "$WATCHDOG_SETUP"
else
    echo "❌ setup-watchdog-systemd.sh not found!"
    exit 1
fi
